...
app.get('/compare_chart', function (req, res){
	console.log('Hi');
	var qstr = "select p.time as p_time, s.time as s_time, p.pm10 as p_pm10, s.d0 as s_pm10, s.u from public_dust as p join sensors_new as s on p.c_num=s.c_num and s.u=501005;"
	mc.query(qstr, function(err, rows, cols){
		if(err){
			throw err;
			res.send('query error: '+qstr);
...
app.get('/compare_chart_option', function (req, res){
	console.log('Hi');
	var dateto = req.query.dateto;
	var datefrom = req.query.datefrom;
	var sensor = req.query.sensor;
	var qstr = "select p.time as p_time, s.time as s_time, p.pm10 as p_pm10, s.d0 as s_pm10, s.u from public_dust as p join sensors_new as s on p.c_num=s.c_num and s.u="+sensor+";"
	mc.query(qstr, function(err, rows, cols){
		if(err){
			throw err;
			res.send('query error: '+qstr);
