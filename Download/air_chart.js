...
app.get('/two_chart', function (req, res){
	console.log('Hi');

	var qstr = "select time, pm10, pm25 from public_dust where date_format(time,'%i')=30 and date_format(time,'%y%m%d')>180529 limit 3000;"
	var qstr2 = "select time, d0, d1 from sensors_new where u=501005 and date_format(time,'%i')=30 and date_format(time,'%y%m%d')>180529 limit 3000;"
	...
	mc.query(qstr2, function(err, rows, cols){
		if(err){
			throw err;
			res.send('query error: '+qstr2);
			return;
		}

		console.log("Got "+rows.length+" records");
		var html2 = "['time', 'd0', 'd1']";
	...
	fs.readFile("air_chart.html", 'utf8', function(err, data2){
			if(err) console.log("file error"+err);

			data2 = data2.replace("<%DATABASE%>", html2);
			res.send(data2);
		});
	});
})
